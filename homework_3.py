...
# Вот абсолютно не понимаю как делать. Следовательно и 5 задачу тоже. Для начала хочу понять, правильно ли пытаюсь мыслить))
    # "получить из list пункта 3 dict,"
        # нужно создать dict и присвоить ему для начала все значения списка, типа dict = {list}
    # ключами которого являются слова,
        # получается dict = {list:count}, дальше уже кодом прописать условие поиска повторений и накручивание счетчика напротив ключа
        # всё, дальше не мыслит голова.
