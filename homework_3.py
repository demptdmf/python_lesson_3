...
print(text)                     # Появились двойные пробелы благодаря этой очистке. Ошибки в этом нет, т.к. шло 3 символа подряд, но как убрать двойные пробелы? text = text.replace('  ', '') ?
print(type(text))
print()
...
# Как оно читается? Не понимаю, потому что очень замудренное кажется.
...
# Почему не работает и как оно читается хочется понять преимущественно, т.к. этот код более короткий с одним правильным циклом
