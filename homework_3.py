...
dict_text = list(dict_text.items())          # Используем парные значения через items, присваивая список для каждой пары "ключ/значение"
dict_text.sort(key=lambda i: i[1])           # сортируем по второму элементу списка (по цифре)
dict_text.reverse()
print(dict_text[:5])
...
# Значит нужно посчитать в словаре все значения с "1" в словаре (value = 1)
# Затем вычесть из общего количества элементов множества
# Значит для начала нужно привести текущий лист к множеству

dict_set = set(dict_text)
# я всё. не могу больше)))


# Считаем количество одинаковых слов:
count = 0
for i in dict_set:
